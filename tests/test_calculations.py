...
def setup_calculations():
    ...
    Calculations.add_calculation(Calculation(Decimal('3'), Decimal('5'), add))
    Calculations.add_calculation(Calculation(Decimal('5'), Decimal('9'), subtract))

def test_add_calculation(setup_calculations):
    """Test adding a calculation to the history."""
    # Create a new Calculation object to add to the history.
    calc = Calculation(Decimal('4'), Decimal('4'), add)
...
def test_get_latest(setup_calculations):
    ...
    assert latest.a == Decimal('5') and latest.b == Decimal('9'), "Did not get the correct latest calculation"
